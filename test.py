...
def train(network, env, episodes=1000, time=500, render=False, plot=True, gif=False):
    ...
    for episode in range(episodes):
        ...
        for t in pbar:
            ...
            network.backward_(discounted_gradients, np.array([0]), clip=10, t=t)
            ...
            max_chars = 160
            pbar_string = f'Episode {episode : 05}: reward={total_reward : 9.3f}; action={action : 2}; conf={probs[action] : 6.3f}; s={s : 6.3f}; ' + \
...
def main():
    ...
    train(network, env, episodes=1000, time=500, render=False, plot=True, gif=False)
